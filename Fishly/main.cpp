/*******************************************************************************
*   Based on                                                                           *
*   PrimeSense NITE 1.3 - Track Pad Sample                                     *
...
int end_eventsocket()
{
  if(eventsock) 
    {
      return close(eventsock);
    }
  return -1;
}


int init_eventsocket(char *hostname, int port)
{
  struct sockaddr_in echoServAddr;

  if ((eventsock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
    {
  ...
  echoServAddr.sin_addr.s_addr = inet_addr(hostname);   /* Server IP address */
  echoServAddr.sin_port        = htons(port); /* Server port */

  if (connect(eventsock, (struct sockaddr *) &echoServAddr, sizeof(echoServAddr)) < 0)
    {
      eventsock=0;
      puts("connect() failed");
      return -1;
